...
const Images = (props) => {
    ...
    const [image, setImage] = useState({id: "", etitle: "", edescription: "", etag: ""})
    ...
    const handleClick = async (e)=>{ 
        ...
        refClose.current.click();
            ...
            await editImage(image.id, image.etitle, image.edescription, image.etag)
        refClose.current.click();
  ...
  const handleScroll = () => {
    if (window.innerHeight + document.documentElement.scrollTop >= document.documentElement.offsetHeight - 100 && !loading && hasMore) {
        ...
        getImagesUser(pg); // Fetch next page of images when scroll reaches the bottom
    }
  };

  useEffect(() => {
    getImagesUser(pg); // Fetch the initial set of images
  ...
  useEffect(() => {
    ...
    return (
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <form className="my-3">
                                <div className="mb-3">
                                    ...
                                    <input type="text" className="form-control" id="etitle" name="etitle" value={image.etitle} aria-describedby="usernameHelp" onChange={onChange} minLength={5} required/>
                                </div>
                                <div className="mb-3">
                                    <label htmlFor="description" className="form-label">Description (more than 5 letters)</label>
                                    <input type="text" className="form-control" id="edescription" name="edescription" value={image.edescription} onChange={onChange} minLength={5} required/>
                                </div>
                        ...
                        <div className="modal-footer">
                            ...
                            <button disabled={image.etitle.length<3 || image.edescription.length<5} onClick={handleClick} type="button" className="btn btn-primary">Update Image</button>
                        </div>
                    </div>
                </div>
            </div>
      ...
      <h2><b>Your Images</b></h2>
      <div className="container mx-2"> 
        {imagesUser.length === 0 && 'No images to display'} 
      </div>
      {imagesUser.map((image) => {
        return <Imageitem key={image._id} updateImage={updateImage} image={image} discover={false} showAlert={props.showAlert} />
      })}
