...
const [image, setImageim] = useState(null);
 ...
 (more than 3 letters)</label>
                  <input
                    type="text"
                    className="form-control"
                    id="etitle"
                    name="etitle"
                    value={imageup.etitle}
 ...
 (more than 5 letters)</label>
                  <input
                    type="text"
                    className="form-control"
                    id="edescription"
                    name="edescription"
                    value={imageup.edescription}
                    onChange={onChange}
                    minLength={5}
                    required
                  />
            ...
            <div className="modal-footer">
              ...
              <button
                disabled={imageup.etitle.length   
 < 3 || imageup.edescription.length < 5}
                onClick={handleClick}
                type="button"
          ...
          <div className='col-md-6'>
            <div className="container">
              <div className="d-flex flex-row-reverse">
                <div>
                  <i className="far fa-solid fa-share fa-lg mx-2 my-4" onClick={async () => {
                    navigator.clipboard.writeText(`https://phodit.vercel.app/posts/${image._id}`);
                    props.showAlert("Link copied to clipboard", "success");
                  }}></i>
                </div>
                <div>
                  {(localStorage.getItem('token')) && (props.discover === false || localStorage.getItem('username') === image.username) && (
                    <i className="far fa-edit fa-lg mx-2 my-4" onClick={async () => {
                      if (!localStorage.getItem('token')) {
                        navigate('/login');
                      } else {
                ...
                <div>
                  {(localStorage.getItem('token')) && (props.discover === false || localStorage.getItem('username') === image.username) && (
                    <i className="far fa-trash-alt fa-lg mx-2 my-4" onClick={async () => {
                      if (!localStorage.getItem('token')) {
                        navigate('/login');
                      } else {
                        await deleteImage(image._id);
                        navigate('/login');
                        props.showAlert("Your image has been deleted successfully", "success");
                      }
                    }}></i>
                  )}
                </div>
              ...
              <div className="text-center" style={{ maxWidth: '800px', margin: '0 auto' }}>
                ...
                <div className="align-items-center">
                  ...
                  <br />
                  {image.parent !== "" && (
                    <Link to={`/posts/${image.parent}/`} onClick={() => { setClick(true); }}>Link to Parent</Link>
                  )}
                </div>
              </div>
            </div>
